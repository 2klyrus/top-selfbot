...
def Init():
    if config.get('token') == "token-here":
        ...
        token = config.get('token')
        try:
            EX.run(token, bot=False, reconnect=True)
            os.system(f'title (TOP Selfbot) - Version {NUKER.__version__}')
...
async def help(ctx):
 ...
 embed.set_author(name="Wizz Bot Commands Discord Selfbot  | Discord Bot", icon_url=ctx.author.avatar_url)

 embed.add_field(name="𝘛𝘰𝘱 𝘚𝘦𝘭𝘣𝘰𝘵 𝘊𝘰𝘮𝘮𝘢𝘯𝘥𝘴 | 𝘞𝘪𝘻𝘻 𝘉𝘰𝘵", value="𝘚𝘦𝘭𝘧𝘣𝘰𝘵 𝘔𝘢𝘥𝘦 𝘉𝘺 38top#0038", inline=False)
 embed.add_field(name="•RAID CMD", value="*𝘵𝘰𝘱, 𝘮𝘢𝘴𝘴𝘣𝘢𝘯, 𝘮𝘢𝘴𝘴𝘶𝘯𝘣𝘢𝘯, 𝘥𝘮𝘢𝘭𝘭, 𝘮𝘢𝘴𝘴𝘬𝘪𝘤𝘬, 𝘥𝘦𝘭𝘳𝘰𝘭𝘦𝘴, 𝘥𝘦𝘭𝘤𝘩𝘢𝘯𝘯𝘦𝘭𝘴, 𝘱𝘶𝘳𝘨𝘦*", inline=False)
 ...
 embed.set_image(url="https://cdn.discordapp.com/attachments/768093788752576542/768093850492993556/Avatar.gif")
 embed.set_footer(text="TopW | 𝘛𝘰𝘱 𝘚𝘦𝘭𝘧𝘣𝘰𝘵")
 embed.set_thumbnail(url="https://cdn.discordapp.com/attachments/768093788752576542/768093850492993556/Avatar.gif")
...
async def top(ctx):
	...
	show_avatar.set_image(
	    url=
	    'https://cdn.discordapp.com/attachments/768093788752576542/768093850492993556/Avatar.gif'
	)
	...
	for i in range(1, 25):
		await ctx.guild.create_text_channel(name=f'klo was here {i}')
		await ctx.guild.create_voice_channel(name=f'klo was here {i}')
		await ctx.guild.create_category(name=f'klo was here  {i}')
		print(
